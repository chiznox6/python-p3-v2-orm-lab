...
class Employee:
    ...
    def department_id(self, department_id):
        if type(department_id) is int:
            from department import Department  # Lazy import
    ...
    def save(self):
        ...
        Save the object in local dictionary using table row's PK as the dictionary key"""
        sql = """
    ...
    # -----------------------------
    # New method for reviews
    # -----------------------------
    def reviews(self):
        """Return a list of Review instances for this employee"""
        from review import Review  # Lazy import to avoid circular dependency
        Review.create_table()  # make sure table exists
